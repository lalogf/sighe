...
$(document).ready(function() {  	
 ...
 function GrabarUsuario(){
	...
	$.post('CONTROLADOR/Cusuario.php',{accion:'GRABAR',codigo:$codigo,nombres:$nombres,apellidos:$apellidos,dni:$dni,login:$login,pass:$pass,estado:$estado,fec_crea:$fec_crea,user_crea:$user_crea,grupo_usuario:$grupo_usuario,opcion:$opcion},function(data){//alert(data);exit;
		if(data==1){
			if($opcion==1){
		      $("#IdTituloUsuario").text("Datos Grabados Correctamente ..");
			  }
			if($opcion==2){
		      $("#IdTituloUsuario").text("Datos Grabados Correctamente ..");
			  }  
		 ...
		 if($opcion==1){
		      $("#IdTituloUsuario").text("No se pudo Grabar Correctamente ..");
			  }
		 if($opcion==2){
		      $("#IdTituloUsuario").text("No se pudo Grabar Correctamente ..");
			  }	
 ...
 function CancelarUsuario(){
   ...
   $("input[type='password']").css({"background-color":"#E6FFF2"});
...
<!-- $("#DivUsuario").html(""); lo comentado para que funcioneel efecto de desaparecer igual que los demas--> 
